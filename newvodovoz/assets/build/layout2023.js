...
!function(t, e) {
        ...
        markSelectedSlides() {
            ...
            if (e)
                for (const i of this.slides)
                    ...
                    i.el.removeAttribute(t)) : (i.el.classList.contains(e) && (E(i.el, e),
    ...
    class ot extends _ {
        ...
        static startFromUrl() {
            ...
            _t.getInstance() || !1 === _t.defaults.Hash)
                return;
    ...
    class rt extends _ {
        ...
        process(t, e) {
            return new Promise(((i,n)=>{
                ...
                c && S(c) || (c = document.createElement("img"),
    ...
    class _t extends g {
        ...
        initLayout() {
            ...
            Ft && jt || (Ft = document.createElement("span"),
            ...
            jt = Ft.cloneNode(),
            null === (t = n.parentElement) || void 0 === t || t.insertBefore(Ft, n),
            null === (e = n.parentElement) || void 0 === e || e.append(jt)),
            this.option("animated") && (P(n, Rt),
            setTimeout((()=>{
        ...
        focus(t) {
            ...
            l.length && (i === jt ? h = l[0] : this.lastFocus !== s && i !== Ft || (h = l[l.length - 1]))) : h = o && "image" === o.type ? s : c || s,
            h && tt(h),
        ...
        hideLoading(t) {
            ...
            if (!e)
                return;
            const i = t.spinnerEl;
            this.isClosing() ? null == i || i.remove() : (E(e, It),
        ...
        static unbind(t, e) {
            ...
            s && n && s.delete(n),
            n && s || (_t.openers.delete(i),
            i.removeEventListener("click", _t.fromEvent))
        }
        static destroy() {
            let t;
...
/*
	Общие скрипты, обслуживающие новый layout (шапка, и т.п.)
*/

// добавление спец. класса body при прокрутке страницы
function activateElementsOnPageScrolling(event) {
	if (window.scrollY>0) {
...
document.addEventListener("scroll", activateElementsOnPageScrolling);
activateElementsOnPageScrolling();

// универсальный переключатель класса элемента
document.querySelectorAll("[data-toggle-class]").forEach((element) => {
	let elemClass = element.getAttribute("data-toggle-class");
	element.addEventListener("click", (event) => {
		event.target.classList.toggle(elemClass);
	});
});

// переключатель класса для тега body
// необходим, например, для выкатывания мобильного меню
document.querySelectorAll("[data-body-toggle-class]").forEach((element) => {
	let bodyClass = element.getAttribute("data-body-toggle-class");
	element.addEventListener("click", (event) => {
		document.getElementsByTagName("body")[0].classList.toggle(bodyClass);
	});
});
...
/*
	Работа со всплывающими формами авторизации
	Во всём функционале используются суффиксы типа VModal ("V" - водовоз)
*/

// параметры стандартного модального окна VModal (Fancybox)
let vModalParams = {
	// инструкции, выполняемые после появления модального окна
	on: {
		done: (fancybox, slide) => {
...
// добавляем обработчики для ссылок с параметром data-v-modal
// обработку делегируем fancybox
// в качестве root указываем родительский элемент, в рамках которого будут обрабатываться элементы с [data-v-modal]
// это необходимо для возможности навешивать обработчики элементам блоков, загруженных асинхронно
function initVModal(root) {
	root = document.querySelector(root);
	root.querySelectorAll("[data-v-modal]").forEach((elem) => {
		elem.addEventListener("click", (event) => {
			event.preventDefault();
			const src = elem.getAttribute("data-v-modal");
			Fancybox.close();
			Fancybox.show([
			  {
			    src: "#vModal--"+src,
...
// фронтэндовая валидация форм
// работает просто, но позволяет кастомизировать сообщения об ошибках
// перебирает необходимые для заполнения поля [required]
function validateVModalForm() {
	document.querySelectorAll(".vModal form").forEach((formElement) => {
		formElement.addEventListener("submit", (event) => {
			let formIsValid = true;
			event.target.querySelectorAll("[required]").forEach((formInputElement) => {
				formInputElement.removeAttribute("data-invalid");
				const inputName = formInputElement.getAttribute("name");
				// стандартная проверка валидности введенных данных
				if (!formInputElement.validity.valid) {
					formInputElement.setAttribute("data-invalid",true);
					formIsValid = false;
				}
			});
			if (!formIsValid) {
				...
				// если прошли валидацию и форма должна быть отправлена ajax-ом
				if (event.target.hasAttribute("data-ajax")) {
					event.preventDefault();
					formAjaxHandler(event.target);
				// стандартная отправка данных формы скрипту (переход на другую страницу)
				} else {
					return true;
				}
			}
...
// кастомная валидация номера телефона
// обрезает префикс страны, удаляет всё кроме цифр и форматирует 999 999 99 99
function phoneValidation(event) {

  // обрежем всё кроме цифр
  curValue = event.target.value.replace ( /[^0-9]/g, '' );
  // если введена одна цифра, пока ничего не делаем
  if (curValue.length<3) {
      return;
  }
  // введено >= 3 цифр:
  // получим первые две цифры
  // https://www.russlandjournal.de/ru/rossija/telefonnye-kody-gorodov-rossii/
  // у городских номеров РФ код региона может начинаться с 8 (например, +7(8..).......), но нет ни одного региона с 88 или 89 в начале кода региона (+7(88.).......).
  firstDigits = curValue.substring(0,2);
  // в зависимости от ситуации скорректируем итоговую строку
  switch (firstDigits) {
    case "88": case "89": case "70": case "71": case "72": case "73": case "74": case "75": case "76": case "77": case "78": case "79":
      correctedValue = curValue.substring(1);
      break;
    default:
      correctedValue = curValue;
  }
  // обрезаем длину
  if (correctedValue.length>10) {
    correctedValue = correctedValue.substring(0,10);
  }
  // форматируем пробелами
  formattedValue = correctedValue.substring(0,3);
  formattedValue += (correctedValue.length>3) ? " "+correctedValue.substring(3,6) : "";
  formattedValue += (correctedValue.length>6) ? " "+correctedValue.substring(6,8) : "";
  formattedValue += (correctedValue.length>8) ? " "+correctedValue.substring(8,10) : "";
  event.target.value = formattedValue;

}

// валидация sms кода
function smsCodeValidation(event) {

  // обрежем всё кроме цифр
  correctedValue = event.target.value.replace ( /[^0-9]/g, '' );

  // обрезаем длину
  if (correctedValue.length>4) {
    correctedValue = correctedValue.substring(0,4);
  }
...
// запуск таймера обратного отсчёта для повторной отправки кода
// перезапускает себя, пока не дойдёт до 0
function smsCodeTimer(selector) {
	document.querySelectorAll(selector).forEach(elem => {
		let curValue = elem.innerHTML;
		if (curValue>0) {
			elem.innerHTML = curValue-1;
			if (typeof(timer)!=='undefined') {
				clearTimeout(timer);
			}
			timer = setTimeout(function() {
				smsCodeTimer(selector);
			}, 1000);
		} else {
			elem.closest(".vModal-userForm__input--smsCodeAction.disabled").classList.remove("disabled");
		}
	});
}
// сбрасываем и запускаем таймер обратного отсчёта для повторной отправки кода
// функция вызывается при каждом открытии модального окна
function resetSmsCodeTimer() {
	document.querySelectorAll('.vModal-userForm__input--smsCodeAction > div > span').forEach(item => {
	    item.innerHTML = 60;
	    item.closest(".vModal-userForm__input--smsCodeAction").classList.add("disabled");
	});
	smsCodeTimer('.vModal-userForm__input--smsCodeAction.disabled > div > span');
}




// т.к. формы загружены в dom при загрузке страницы, можно сразу добавить им обработчики
// инициируем показ модальных окон
initVModal("body");
// подключаем модерацию форм
validateVModalForm();
// подключаем валидацию инпута номера телефона
document.querySelectorAll('.vModal form [autocomplete="tel"]').forEach(item => {
    item.addEventListener('input', event => {
        phoneValidation(event);
    });
});
// подключаем валидацию инпута номера телефона
document.querySelectorAll('.vModal-userForm__input--smsCode input').forEach(item => {
    item.addEventListener('input', event => {
        smsCodeValidation(event);
    });
});
...
// вызывается в validateVModalForm() (после успешной валидации)
async function formAjaxHandler(formElement) {
	const src = formElement.getAttribute("action");
	const method = formElement.getAttribute("method");
  const formattedFormData = new FormData(formElement);// uh add
  let vModalParamsTmp = vModalParams;// uh add

	let preloaderWrap = document.querySelectorAll(".vModal");

	for (var i = 0; i < preloaderWrap.length; ++i) {
    preloaderWrap[i].classList.add("is-loading");
	}
	formElement.classList.add("loading");
	formElement.querySelector("[type='submit']").setAttribute("disabled",true);

	// добудем введенный номер телефона, он может фигурировать при обработке результатов запросов

	let $phone = (formElement.querySelector("[name='phone']") !== null && typeof(formElement.querySelector("[name='phone']"))!=='undefined' && formElement.querySelector("[name='phone']").value.length>0) ? formElement.querySelector("[name='phone']").value : false;
	// console.log(phoneNumber);

	// блок для вывода ошибок в результате запроса
	const formResponseErrorStr = formElement.querySelector(".form-error-str.response");
	formResponseErrorStr.innerHTML = '';
	...
	maintry: try {
		...
		if (typeof(Response.code)==='undefined') {
			formResponseErrorStr.innerHTML = 'Неизвестная ошибка';
			break maintry;
		}

		// авторизация
		if (Response.method=="user_phone") {
			switch(Response.code) {
				case 'captcha_error':
					// Не заполнили капчу
					formResponseErrorStr.innerHTML = (typeof(Response.msg)!=='undefined') ? Response.msg : "Пройдите проверку reCaptcha";
					break;

				case 'phone_format_error':
					// Неверный формат номера телефона
					formResponseErrorStr.innerHTML = (typeof(Response.msg)!=='undefined') ? Response.msg : "Неверный формат номера телефона";
					break;

				case 'multiple_users':
					// несколько пользователей с таким телефоном
					formResponseErrorStr.innerHTML = (typeof(Response.msg)!=='undefined') ? Response.msg : "Увы, Ваш номер телефона прикреплён к нескольким клиентам";
					break;

				case 'spam_request':
					// Слишком частая отправка sms
					formResponseErrorStr.innerHTML = (typeof(Response.msg)!=='undefined') ? Response.msg : "Слишком много запросов на отправку sms, попробуйте через 5 минут";
					break;

				case 'sms_send_fail':
					// Ошибка отправки смс. Сервсис не ответил или ошибка отправки
					formResponseErrorStr.innerHTML = (typeof(Response.msg)!=='undefined') ? Response.msg : "Ошибка отправки SMS";
					break;

				case 'sms_send_successful': // покупателю или пользователю отправлен sms
					// закрываем текущее модальное окно
					Fancybox.close();
					// сбрасываем заполнение формы
					formElement.reset();

					// заполним поля
					if(!!Response.phone){
						document.querySelector("#vModal--userSmsCode .js_user_check_sms_phone").value = Response.phone;
					}
					if(!!Response.new_user){
						document.querySelector("#vModal--userSmsCode .js_user_check_sms_phone_new_user").value = Response.new_user;
					}
					if(!!Response.phone_text){
						document.querySelector("#vModal--userSmsCode .js_user_check_sms_for_phone").innerHTML = Response.phone_text;
					}

					// открываем модальное окно для ввода смс-кода
					vModalParamsTmp.id = 'userSmsCode';
					Fancybox.show([
						{
							src: "#vModal--userSmsCode",
							type: "inline",
						},
					],vModalParamsTmp);
					break;
		...
		// ввод смс кода
		if (Response.method=="user_chech_sms") {
			switch(Response.code) {
				case 'sms_wrong_format': // Неверный формат кода из sms
					formResponseErrorStr.innerHTML = (typeof(Response.msg)!=='undefined') ? Response.msg : "Неверный формат кода из sms";
					break;
          ...
          epr_user_cart_fields();

					break;

				case 'wrong_sms_code': // sms код неверный
					formResponseErrorStr.innerHTML = (typeof(Response.msg)!=='undefined') ? Response.msg :  "Неверный код";
					break;

				case 'auth_success': // sms код верный, Пользователь авторизован
					$('.js_user_lk').removeClass('hidden');
					$('.js_user_login').addClass('hidden');
					...
					// открываем модальное окно успешной авторизации
					vModalParamsTmp.id = 'userLoginCompleted';
					Fancybox.show([
						{
							src: "#vModal--userLoginCompleted",
							type: "inline",
						},
					],vModalParamsTmp);
          
          //$('.js-cart_guest').remove();
          ...
          // изменения для постоянных клиентов
          $.ajax({
            url: '/udata/users/isNewClient.json',
            dataType: 'json',
            async: true,
            success: function(response) {
              if(response.hasOwnProperty('is_new_client') && response.is_new_client == 0){
                $('.js_banners_wrap').remove();
                if($('.not_old_client').length > 0){
                  window.location.replace("/promonabory-dlya-klientov2/");
                }
              }
            }
          });
				...
				case 'erpUpdate_error': // Ошибка обновления данных. (при обновлении данных в ERP)
				case 'erpUpdate_unknown': // Неизвестная ошибка. (при обновлении данных в ERP)
				case 'erpCheck_error': // Ошибка определения пользователя. (при проверке пользователя в ERP)
				case 'erpReg_error': // Ошибка регистрации пользователя. (при регистрации пользователя в ERP)
				case 'erpReg_unknown': // Неизвестная ошибка. (при регистрации пользователя в ERP)
				case 'erpCheck_unknown': // Неизвестная ошибка. (при проверке пользователя в ERP)
				case 'erpNewCheck_error': // Ошибка определения пользователя. (при проверке пользователя в ERP)
				case 'erpNewCheck_unknown': // Неизвестная ошибка. (при проверке пользователя в ERP)
					formResponseErrorStr.innerHTML = "Ошибка при обработке запроса. Пожалуйста, свяжитесь с нами по телефону.";

          epr_user_cart_fields();
					break;

				case 'erpNewCheck_needReg': // Необходимо дополнить ваши данные. (при проверке пользователя в ERP)
				case 'erpNewCheck_needFullReg': // Необходимо зарегистрироваться. (при проверке пользователя в ERP)
					// закрываем текущее модальное окно
					Fancybox.close();
					// сбрасываем заполнение формы
					formElement.reset();
					...
					// заполним поля
					if(!!$phone){
						document.querySelector("#vModal--userRegistration .js_user_registrate_phone").value = $phone;
					}
					if(!!Response.phone_format){
						document.querySelector("#vModal--userRegistration .js_user_registrate_phone_text").innerHTML = Response.phone_format;
					}

					// дополнить параметры из статуса erpNewCheck_needReg
          if(!!Response.fname){
            document.querySelector("#vModal--userRegistration .vModal-userForm__input--name input").value = Response.fname;
          }
          if(!!Response.lname){
            document.querySelector("#vModal--userRegistration .vModal-userForm__input--last-name input").value = Response.lname;
          }
          if(!!Response.father_name){
            document.querySelector("#vModal--userRegistration .vModal-userForm__input--additional-name input").value = Response.father_name;
          }
          if(!!Response.email){
            document.querySelector("#vModal--userRegistration .vModal-userForm__input--email input").value = Response.email;
          }

					// открываем модальное окно регистрации
					vModalParamsTmp.id = 'userRegistration';
					Fancybox.show([
						{
							src: "#vModal--userRegistration",
							type: "inline",
						},
					],vModalParams);
		...
		if (Response.method=="registrate_by_phone_do") {
			switch(Response.code) {
        ...
        * 50 - "Пользователь с таким телефоном уже есть на сайте"*/
				case 'phone_format_error': // Неверный формат номера телефона
				case 'wrong_email_format': // Неверный формат e-mail
				case 'email_not_unical': // Пользователь с таким e-mail уже существует
				case 'multiple_users': // Увы, Ваш номер телефона прикреплен к нескольким клиентам
				case 'erpUpdate_error': // Ошибка обновления данных. (при обновлении данных в ERP)
				case 'erpUpdate_unknown': // Неизвестная ошибка. (при обновлении данных в ERP)
					formResponseErrorStr.innerHTML = (typeof(Response.msg)!=='undefined') ? Response.msg :  "Ошибка при обработке запроса. Пожалуйста, свяжитесь с нами по телефону.";

          epr_user_cart_fields();

					break;

				case 'reg_success': // Пользователь успешно зарегистрирован
          $('.js_user_lk').removeClass('hidden');
          $('.js_user_login').addClass('hidden');
					...
					// открываем модальное окно регистрации
          vModalParamsTmp.id = 'userRegistrationCompleted';
					Fancybox.show([
					  {
					    src: "#vModal--userRegistrationCompleted",
					    type: "inline",
					  },
					],vModalParamsTmp);

					document.querySelector(".v-header-darkline__login.lk").classList.remove("hidden");
					document.querySelector(".v-header-darkline__login:not(.lk)").classList.add("hidden");
		...
		// Запрос на уточнение кол-во бутылей у клиента
		if (Response.method=="sendBottleInfo") {
			switch(Response.code) {
          case 'phone_format_error': // Обиюка формата телефона.
					formResponseErrorStr.innerHTML = (typeof(Response.msg)!=='undefined') ? Response.msg :  "Ошибка при обработке запроса. Пожалуйста, свяжитесь с нами по телефону.";
					break;

				case 'send_successful': // Запрос на утонение информации о количестве бутылей успешно отправлен
					// закрываем текущее модальное окно
					Fancybox.close();
					// сбрасываем заполнение формы
					formElement.reset();

					// открываем модальное окно успешной отправки
          vModalParamsTmp.id = 'checkBottleRequest';
					Fancybox.show([
					  {
					    src: "#vModal--checkBottleRequest",
					    type: "inline",
					  },
					],vModalParamsTmp);
          break;
	...
	for (var i = 0; i < preloaderWrap.length; ++i) {
    preloaderWrap[i].classList.remove("is-loading");
	}
...
// если нажали на кнопку "Получить новый код"
document.querySelectorAll("[data-resend-code]").forEach((element) => {
  element.addEventListener("click", (event) => {
    event.preventDefault();
    const root = event.target.closest("form");
    document.querySelector('#vModal--userLogin form [name="phone"]').value = root.querySelector('[name="phone"]').value;
    document.querySelector('#vModal--userLogin form [type="submit"]').click();
  });
});
/*document.addEventListener("click","[data-resend-code]",(event) => {
	event.preventDefault();
	const root = event.target.closest("form");
	document.querySelector('#vModal--userLogin form [name="phone"]').value = root.querySelector('[name="phone"]').value;
	document.querySelector('#vModal--userLogin form [type="submit"]').click();
